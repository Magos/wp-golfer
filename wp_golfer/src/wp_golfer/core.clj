...
(defn search 
  ...
  (if (some (partial contains? f-parents) (keys b-parents))
    ...
    (let [fwd-article (first f-frontier)
          ...
          fwd-children (filter not-in-f (wp-golfer.retrieval/child-articles fwd-article))
          bck-parents (filter not-in-b (wp-golfer.retrieval/parent-articles bck-article))
